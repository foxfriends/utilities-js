...
exports.type = __webpack_require__(2);
...
exports.objDisplay = __webpack_require__(7);
...
var flag = __webpack_require__(0),
    ...
    objDisplay = __webpack_require__(7);
...
let MatchError = class MatchError extends Error {};
let PatternError = class PatternError extends Error {};
let InternalMatchError = class InternalMatchError extends Error {};
...
function extractor(extractors, inst) {
  ...
  if (extracted.length !== extractors.args.length) {
    throw new MatchError(`Expected ${extracted.length} parameters, but found ${extractors.args.length}`);
  }
    ...
    switch (typeof ext) {
      case 'string':
        if (ext[0] === '=') {
          ...
          return ext === '*' ? obj : _extends({}, obj, { [ext]: extracted[i] });
        }
      case 'object':
        return _extends({}, obj, extractor(ext, extracted[i]));
    }
  }, {});
}
...
function parse(pat) {
  ...
  if (pat.indexOf('(') !== -1 && pat[pat.length - 1] !== ')') {
    fail();
  }
 ...
 */
function pattern(strings, ...values) {
